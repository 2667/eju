...
- (void)initFirstTableViewWithFrame:(CGRect)frame {
    _firstTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width / 2, frame.size.height * 0.4) style:UITableViewStylePlain];
    ...
    if (tableView == _firstTableView) {
        ...
        switch (indexPath.section) {
            case 0:
            {
                ...
                cell.textLabel.highlightedTextColor = [UIColor colorWithRed:0.953 green:0.647 blue:0.212 alpha:1.0];
                break;
            }
            default:
                break;
        ...
        selectLine.backgroundColor = [UIColor colorWithRed:0.953 green:0.647 blue:0.212 alpha:1.0];
...
- (void)showSingleOrDouble {
    if (_leftArray.count <= 0) {
        ...
        _secondTableView.frame = CGRectMake( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT * 0.4);
    } else {
        [_firstTableView setHidden:NO];
        _secondTableView.frame = CGRectMake(SCREEN_WIDTH / 2, 0, SCREEN_WIDTH, SCREEN_HEIGHT * 0.4);
    }
}
